...
function createBeam(width: number, height: number): Beam {
  ...
  return {
    ...
    opacity: isDark ? 0.18 + Math.random() * 0.16 : 0.34 + Math.random() * 0.16,
...
export default function BeamsBackground({ className, intensity = "strong", children }: AnimatedGradientBackgroundProps) {
  ...
  useEffect(() => {
    ...
    function resetBeam(beam: Beam, index: number, totalBeams: number) {
      ...
      beam.hue = (isDark ? 190 : 250) + (index * 70) / totalBeams
  ...
  return (
    <div className={cn("relative min-h-screen w-full overflow-hidden bg-neutral-50 dark:bg-neutral-950", className)}>
